...
(defn prepare-new-gaps [new-gaps ranges from chat-ids]
  (into
   {}
   (comp
    (map (fn [chat-id]
...
(fx/defn update-gaps
  [{:keys [db]}]
  (let [{:keys [topics from] :as request} (get db :mailserver/current-request)
        chat-ids          (into #{}
                                (comp
        ...
        ranges            (:mailserver/ranges db)
        prepared-new-gaps (prepare-new-gaps new-gaps ranges from chat-ids)]
    {:db
